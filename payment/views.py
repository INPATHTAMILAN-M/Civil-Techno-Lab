        ...
        if serializer.is_valid():            
            ...
            for i_test in serializer:
                ...
                if i_test.test.material_name.material_name == 'Rebound Hammer':
                    data = data.replace('<td colspan="2">&nbsp;</td>', f'<td colspan="2"><img alt="Logo" src="{settings.DOMAIN_NAME}/static/header-hammer.png" style="width:100%" /> </td>',1)
                    data = data.replace('ULR:',f"ULR: {i_test.ulr}")
                else:                
                    data = data.replace('<td colspan="2">&nbsp;</td>', f'<td colspan="2"><img alt="Logo" src="{settings.DOMAIN_NAME}/static/header.gif" style="width:100%" /> </td>',1)
...
def qr(request):
    ...
    # Add data to the QR code
    qr.add_data(f"{settings.DOMAIN_NAME}/create_invoice/")
    qr.make(fit=True)
